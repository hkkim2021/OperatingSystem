...
wait(void)
{
  ...
  for(;;){
    ...
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->parent != curproc && p->tmaster !=curproc)
        continue;
      havekids = 1;
      if(p->state == ZOMBIE){
        // Found one.
...
thread_create(thread_t* thread, void*(*start_routine)(void *), void *arg)
{
  ...
  //tmaster가 아니면 thread생성 불가능
  if(curproc->tmaster !=curproc){
    return -1;
  }
  ...
  if(copyout(np->pgdir, curproc->sz, ustack, 8) < 0){
    return -1;
  }
